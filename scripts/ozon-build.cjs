...
/* КАТЕГОРИИ: ДВЕ ШТУКИ, И ЭТО НАМЕРЕННО (27.08.2026).
   Раньше Озон-категория ЗАМЕНЯЛАСЬ ВБ-категорией, чтобы фильтр работал одинаково на обеих
   площадках. Продавец попросил вернуть родные названия Озона: одинаковые подписи на двух
   площадках мешали работать. Но связка «Всего» держится именно на общей классификации,
   поэтому ВБ-категория никуда не девается — она живёт в `wbCategory`:
     · `category`   — как называется у Озона (задаёт `ozon-category.cjs` из отчёта аналитики);
     · `wbCategory` — категория из ВБ-каталога, КЛЮЧ СВЯЗКИ для режима «Всего».
   Озон-категории крупнее (269 «Каталка» против ВБ «Пушкары»/«Толокары» и т.п.), поэтому
   каноничной для «Всего» остаётся ВБ-категория — она детальнее и покрывает обе площадки.
   Если родной категории ещё нет (каталог собран до этой правки) — подставляем ВБ-категорию,
   чтобы фильтр не обнулился. Не заменяй `category` обратно. */
ozCat.forEach(c=>{
  if(c.wbSku && wbCatBySku[c.wbSku]) c.wbCategory = wbCatBySku[c.wbSku];
  if(!c.category || c.category==='Без категории') c.category = c.wbCategory || 'Без категории';
});
