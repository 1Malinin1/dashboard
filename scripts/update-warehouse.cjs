...
/* «Склад ответхранения Евросиб (магазины)» — товар на ответственном хранении под РОЗНИЧНЫЕ
   магазины, а не наш отгрузочный запас. Имя содержит «Евросиб», поэтому правило ниже слило бы
   его со «Складом Евросиб» и раздуло остаток, доступный к отгрузке на площадки. Возвращаем имя
   как есть — оно не попадёт в KNOWN_WH и честно уйдёт в блок «не посчитано».
...
const mpCols=[]; const mpBy={};      // незнакомые колонки — не считаем (белый список KNOWN_WH)
...
if(layout==='A'){
  ...
  for(let i=iCode+1;i<H.length;i++){ const h=H[i]; if(!h) continue; if(/^Итог/i.test(h)) break;
    ...
    /* СЧИТАЕМ ТОЛЬКО ИЗВЕСТНЫЕ СКЛАДЫ — БЕЛЫЙ СПИСОК, А НЕ ЧЁРНЫЙ (правило продавца 31.08.2026).
       В выгрузке начали появляться колонки вроде «Склад ОЗОН ПФО», «Склад ОЗОН СФО»,
       «Склад РВБ ПФО». Продавец подтвердил: этого товара нет ни у него на складе, ни на
       площадке — это баг выгрузки, считать его нельзя. Раньше любая незнакомая колонка молча
       становилась новым складом и попадала в остаток; теперь берём ровно четыре своих склада,
       всё остальное складываем в «не посчитано» и печатаем списком, чтобы это было видно.
       Явный ярлык --wh белый список обходит: это осознанное решение оператора. */
    const w=canonWh(h);
    if(!whForced && !KNOWN_WH.has(w)){ mpCols.push({i,name:h}); continue; }
    cols.push({i,name:w}); }
...
if(mpCols.length){
  console.log('\nНЕИЗВЕСТНЫЕ КОЛОНКИ — НЕ ПОСЧИТАНЫ (считаем ровно четыре своих склада):');
  mpCols.forEach(c=>console.log('   '+c.name+': '+Math.round(mpBy[c.name]||0).toLocaleString('ru-RU')+' шт'));
  console.log('   Если среди них появится ваш реальный склад — добавьте его в KNOWN_WH.');
}
